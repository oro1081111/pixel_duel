...
import {CARD_DEFS, EFFECTS} from './cards';
...
function chooseLowestValueDieIndex() {
    return createAdeptHeuristic().chooseLowestValueDieIndex();
}

function chooseAdeptFateDiceIndices() {
    return createAdeptHeuristic().chooseFateDiceIndices();
}

function chooseAdeptFrostDieIndex() {
    return createAdeptHeuristic().chooseFrostDieIndex();
}

function chooseAdeptIllusionTargetArea() {
    return createAdeptHeuristic().chooseIllusionTargetArea();
}

function scoreCardForAdept(card: GameCard, areaIdx = -1) {
    return createAdeptHeuristic().scoreCard(card, areaIdx);
}

function chooseAdeptPlay() {
    return createAdeptHeuristic().choosePlay();
}

function chooseAdeptPlayCount() {
    return createAdeptHeuristic().choosePlayCount(S.inPreparationPhase);
}

function chooseAdeptRollCount(rollOptions: number[]) {
    return createAdeptHeuristic().chooseRollCount(rollOptions);
}

function scoreActivationForAdept(effectId: string) {
    return createAdeptHeuristic().scoreActivation(effectId);
}

/*
 * 每個效果的中文標籤只供 UI／戰報顯示。
 * AI 評分一律使用 effectId，不再讓翻譯或文案修改影響決策。
 */
...
async function aiActivationLoopStep() {
    ...
    const scored = acts.map(act => ({
        act,
        score: scoreActivationForAdept(act.effectId) + Math.random() * 0.1,
...
async function aiDoBuyPhase() {
  ...
  score: scoreCardForAdept(c) - price * 1.35 + Math.random() * 0.1,
