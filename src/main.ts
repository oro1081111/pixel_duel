...
function getCardFrameStyleVars(size: 'board' | 'hand' | 'market') {
    ...
    // board
    return '--card-w: 75px; --card-h: 90px; --header-h: 22px; --chip: 15px; --chip-font: 8px; --title-font: 11px;';
}
...
function attachCardTooltip(
    ...
    cardEl.addEventListener('mouseleave', hideGlobalTooltip);
}
...
function initGame() {
  ...
  phaseHint = '準備階段：後手請先從手牌打出 1 張到場地';
...
function nextPhase() {
  ...
  if (currentPhaseIndex === 0) { // Play Phase
      ...
      if (deck.length > 0 && buyDeckDrawCount < 1) {
          phaseHint = '購買階段：請先從牌庫抽取第 1 張免費牌';
          render();
          return;
      }
...
function handleJudging() {
    ...
    if (breakthroughCardIdx !== -1 && p.hp <= 3) {
        addLog(`[突破] 生命 <= 3，判定獲得的所有數值 (攻/防/魔/金) 翻倍！`);
        for (let i = 0; i < 3; i++) {
            areaSums[i] *= 2;
            areaDefense[i] *= 2;
            areaMagic[i] *= 2;
    ...
    p.activeAreaEffects.forEach((card, aIdx) => {
        if (getEffectiveEffectId(p, aIdx) === 'surge') {
            if (areaMagic[aIdx] > 0) {
                ...
                areaMagic[aIdx] *= 2;
                addLog(`[湧動] 區域 ${aIdx + 1} 魔力翻倍: ${bonus} -> ${areaMagic[aIdx]}`);
            }
        }
    });
...
function handleDamagePhase() {
    ...
    p.hp -= totalDamage;
    phaseHint = `總共受到 ${totalDamage} 點傷害`;
    if (totalDamage === 0) {
        addLog(`完美防禦！未受到任何傷害`);
    } else {
...
function handleBuyPhase() {
    ...
    phaseHint = deck.length === 0
        ? '購買階段：牌庫已空，可直接結束回合或購買市場牌'
        : '請先從牌庫抽取第 1 張免費牌，之後可自由購買市場牌';
    render();
}
...
function renderMarketPanel(typeColors) {
    ...
    if (canDraw) {
        deckCard.classList.add('ring-4', mustDraw ? 'ring-emerald-400' : 'ring-indigo-300');
    ...
    slots.forEach(({idx, price, label}) => {
        ...
        if (c) {
            ...
            if (canBuy) {
                cardEl.classList.add('ring-4', 'ring-amber-300');
...
function playToBoard(areaIdx) {
    ...
    if (inPreparationPhase) {
        phaseHint = '準備完成，點擊「開始遊戲」進入正式流程';
    } else {
        phaseHint = `已出牌：${p.cardsPlayedThisTurn}/3`;
    }
...
function render() {
    ...
    if (winner) {
        ...
        winScreen.className = 'fixed inset-0 bg-slate-900 z-50 flex flex-col items-center justify-center p-8';
        winScreen.innerHTML = `
            <div class="text-xs font-black text-slate-400 mb-2 uppercase tracking-[0.3em]">對局結束</div>
            <div class="text-6xl font-black text-white mb-2 tracking-tighter italic uppercase">${winner} 獲勝</div>
            <button onclick="location.reload()" class="mt-8 bg-indigo-600 text-white px-12 py-3 rounded-full font-bold shadow-xl">重新開始</button>
        `;
    ...
    if (luckySelectionMode) {
        displayPhaseHint = "[幸運] 發動中，請選擇一顆骰子移除";
    }
    if (illusionSelectionMode) {
        displayPhaseHint = "[幻象幽影] 發動中，請點擊對手的任一招式卡";
    }
...
function renderPlayerArea(idx) {
    ...
    stack.forEach((card, cIdx) => {
        ...
        if (effId === 'lucky' && isActiveEffect && isCurrent && currentPhaseIndex === 1 && (diceResults.length === 0 || luckySelectionMode)) {
            cardEl.classList.add('ring-4', 'ring-lime-400', 'z-40');
        }
        ...
        if (isCurrent && isTop && isActiveEffect) {
            ...
            if (currentPhaseIndex === 5 && effId === 'charge') {
                if (p.magic >= 2 && !p.chargeUsedIndices.includes(aIdx) && !isMirageBlocked) {
                    ...
                    cardEl.classList.add('ring-4', isSource ? 'ring-amber-500' : 'ring-indigo-400', 'cursor-pointer');
                ...
                if (p.magic >= 1 && !isMirageBlocked) {
                    cardEl.classList.add('ring-4', 'ring-emerald-400', 'cursor-pointer');
                ...
                if (!p.amplifyUsedIndices.includes(aIdx)) {
                    cardEl.classList.add('ring-4', 'ring-blue-400', 'cursor-pointer');
                ...
                if (!p.fateUsedIndices.includes(aIdx) && diceRolled && !luckySelectionMode) {
                    cardEl.classList.add('ring-4', fateSelectionMode ? 'ring-amber-500' : 'ring-indigo-400', 'cursor-pointer');
                ...
                if (p.magic >= 3 && !p.barrierUsedIndices.includes(aIdx) && !isMirageBlocked) {
                    cardEl.classList.add('ring-4', 'ring-indigo-400', 'cursor-pointer');
                ...
                if (p.magic >= 2 && !isMirageBlocked) {
                    cardEl.classList.add('ring-4', 'ring-blue-300', 'cursor-pointer');
                ...
                if (!p.thrustUsedIndices.includes(aIdx)) {
                    cardEl.classList.add('ring-4', 'ring-rose-400', 'cursor-pointer');
                ...
                if (!p.frostUsedIndices.includes(aIdx) && diceRolled && !luckySelectionMode) {
                    ...
                    cardEl.classList.add('ring-4', isSource ? 'ring-amber-500' : 'ring-blue-400', 'cursor-pointer');
                ...
                if (p.magic >= 2 && !p.magicLuckUsedIndices.includes(aIdx) && !isMirageBlocked) {
                    cardEl.classList.add('ring-4', 'ring-purple-400', 'cursor-pointer');
                ...
                if (p.magic >= 1 && !p.illusionUsedIndices.includes(aIdx) && !isMirageBlocked && hasCopyableCard) {
                    cardEl.classList.add('ring-4', 'ring-teal-400', 'cursor-pointer');
                ...
                if (p.magic >= 2 && !isMirageBlocked) {
                    cardEl.classList.add('ring-4', 'ring-yellow-300', 'cursor-pointer');
        ...
        if (!isCurrent && isTop && isActiveEffect && illusionSelectionMode && card.effectId !== 'lucky' && card.effectId !== 'fate') {
            cardEl.classList.add('ring-4', 'ring-teal-500', 'cursor-pointer', 'shadow-2xl', 'z-50');
